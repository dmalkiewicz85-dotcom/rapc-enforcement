...
import { afterApproval, noticeReadiness } from './notices.js'
...
export async function decideEvent(eventId, decision, user, settings = {}) {
  ...
  // Refuse before writing anything if the notice could not be produced:
  // the Board fixes configuration first rather than approving half-way.
  if (plan.eventPatch.status === 'APPROVED') {
    const missing = noticeReadiness({
      event: { ...ctx.event, ...plan.eventPatch }, rule: ctx.rule, settings,
      ownerEmail: ctx.violation.owner_email_snapshot,
    })
    if (missing.length) {
      const err = new Error(`Cannot approve yet — the notice needs: ${missing.join(', ')}`)
      err.missing = missing
      throw err
    }
  }
  ...
  if (plan.eventPatch.status === 'APPROVED') {
    steps.push(['notice', () => afterApproval({
      ...
      owner: data.OWNERS.find(o => o.id === ctx.violation.owner_id) ?? null, settings, user, at,
    })])
  }
