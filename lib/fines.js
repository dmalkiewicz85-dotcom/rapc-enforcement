...
import { readTabs, readSettings, appendRows, updateRow, updateRows, writeSteps } from './sheets.js'
import { sendMail } from './gmail.js'
import { NEEDS_INPUT } from './seed.js'
import { can } from './schema.js'
import { newId } from './ids.js'
import { nowISO, formatDate, todayISO } from './time.js'
...
// Emails the Excel report to the configured PM recipient from the HOA
// account, then records those fines as SENT_TO_PM with the message id.
// Deliberately a manual, Board-initiated action (spec).
export async function emailPmReport(user) {
  if (!can(user, 'pm_reports')) throw new Error('Your role cannot send the PM report')
  const settings = await readSettings()
  const to = settings.pm_report_recipient_email
  if (!to || to === NEEDS_INPUT) throw new Error('HOA_SETTINGS.pm_report_recipient_email is not set')
  const fines = reportable(await loadFines())
  if (!fines.length) throw new Error('No fines are ticked for the report')
  const rows = buildPmReportRows(fines)
  const filename = `RAPC-PM-Fine-Report-${todayISO()}.xlsx`
  const hoa = settings.hoa_display_name || settings.hoa_legal_name || 'HOA'
  const total = rows.reduce((s, r) => s + r['Fine Amount'], 0)
  const sent = await sendMail({
    to,
    subject: `${hoa} — Board-approved fines for assessment (${rows.length})`,
    text: `Attached are ${rows.length} Board-approved fine(s) totalling $${total.toFixed(2)} for assessment to the owner accounts listed.

Sent from the ${hoa} enforcement system by ${user.name}.`,
    attachments: [{ filename, mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', content: pmReportXlsx(rows) }],
  })
  const marked = await markSentToPm(fines.map(x => x.id), user, { messageId: sent.id })
  return { ...marked, to, gmailMessageId: sent.id }
}
