...
// Same as updateRow for many ids at once: one read, one batchUpdate.
// patches: [{ id, patch }]. Throws before writing if any id is missing.
export async function updateRows(tab, patches) {
  if (!patches.length) return []
  const idCol = TABS[tab][0]
  const headers = TABS[tab]
  const rows = await readTab(tab)
  const byId = new Map(rows.map(r => [String(r[idCol]), r]))
  const data = []
  const out = []
